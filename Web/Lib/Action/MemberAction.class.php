...
class MemberAction extends BaseAction
{
    ...
    function _initialize()
    {
        ...
        $this->wxconfig['callback'] = 'http://' . $_SERVER['HTTP_HOST'] . __ROOT__ . "/index.php?m=Member&a=wxcallback";
...
//zxy注册
    public function run_register(){
        if(intval(C('MOBILE_VERIFY')) == 1){self::check_verify(1);}        //???
        $user = M('user');
        $model = D("Member");
        // p($_POST);
        $pdresult = $model->pd();
...
//循环出购物车 写进数据库
        if ($trade_type == 1 || $trade_type == 4 || $trade_type == 5) {
            ...
            for ($i = 0; $i < count($_POST['id']); $i++) {
                ...
                $data['gid'] = $_POST['id'][$i];
                $data['uid'] = $_SESSION['dami_uid'];
                $data['price'] = $_POST['price'][$i];//必须,信任客户端表单可以改写哈$_POST['price'][$i]
...
//print_r($userinfo);
    ...
    function wxcallback()
    {
        import('ORG.Util.Wxlogin');
        $lasturl = urlencode(htmlspecialchars('http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']));
        $wx = Oauth_qq::getInstance($this->wxconfig);
...
//创建帐号
    function qqcreate()
    {
        ...
        unset($data['money']);//禁止修改money
        unset($data['is_lock']);//禁止修改锁定状态
        if ($data['realname'] == '' || ($data['openid'] == '' && $data['qid'] == '')) {
            $this->error('参数错误!');
            exit();
        }
