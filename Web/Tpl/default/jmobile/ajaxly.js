...
function showlist(data)
{
	$('#list').html('');
		...
		xmlTitle = data.getElementsByTagName("Title");
		//输出
		for (i=0;i<xmlContent.length;i++) {  
			...
			if (Content == "没有留言") {
			   ...
			   var str = "<li ><h2>标题:"+Title+"</h2><h3>留言时间:"+PostTime+" 留言者：<font color='red'>"+Author+"</font></h3><p>"+Content+"</p><p class='reply'>"+reContent+"</p><p><a href='/DM/index.php?m=Guestbook&a=compile&id="+PostTime+"'>编辑  </a>|<a href='/DM/index.php?m=Guestbook&a=delete&id="+PostTime+"'>删除</a></p></li>"
			}
            $("#list").append(str);
		}
		//重新渲染UL输出
...
var s_data = "tel="+tel+"&author="+Author+"&content="+Content+"&title="+Title;
$.ajax({
async:false,
type: 'post',
